...
public class CurrencyExchangeJsoup implements CurrencyExchange{
   public Temporary temporary = new Temporary();
    CurrencyExchangeJsoup(Temporary temporary){
        this.temporary = temporary;
    }
    CurrencyExchangeJsoup(){
        ...
        HashMap<Currency, Double> currencyRelations = new HashMap<>();
        {
            for (Currency symbol : symbols) {
                try {
                    ...
                    dblValue = Math.round(temporary.getValue() * dblValue * 100.0) / 100.0;
                    System.out.println(temporary.getValue() + "\"" + base.name() + "\"" + " = " + dblValue + "\"" + symbol.name() + "\"");
